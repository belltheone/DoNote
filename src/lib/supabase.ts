...
export async function requestSettlement(creatorId: string, amount: number): Promise<{ success: boolean; message: string; settlementId?: string }> {
    ...
    const { data, error } = await supabase
        ...
        .insert({
            ...
            status: 'pending',
            memo: `정산 요청: ${new Date().toLocaleDateString('ko-KR')}`,
        })
        .select('id')
        .single();
    ...
    return {
        success: true,
        message: '정산 요청이 완료되었습니다. 영업일 기준 3-5일 내 처리됩니다.',
...
// 정산 거절 (관리자용)
export async function rejectSettlement(settlementId: string, reason: string): Promise<boolean> {
    const { error } = await supabase
        .from('settlements')
        .update({
            status: 'rejected',
...
export async function getPendingSettlements(): Promise<{
    ...
    if (error) {
        console.error('대기 정산 목록 조회 오류:', error);
        return [];
    }
